(() => {
  ...
  const HOVER_TONE_PRESETS = [
    { id: 'neutral', label: '기본', prompt: '' },
    {
      id: 'friendly',
      label: '친근',
      prompt: '답변을 따뜻하고 격려하는 톤으로 작성해줘.',
    },
    {
      id: 'polite',
      label: '정중',
      prompt: '답변을 정중하고 비즈니스 용어를 활용한 문체로 작성해줘.',
    },
    {
      id: 'concise',
      label: '간결',
      prompt: '핵심만 간결하게 설명해줘. 필요 시 Bullet으로 정리해줘.',
    },
  ];
...
`;
  ...
  function setDropdownOpen(dropdownEl, open) {
    ...
    if (open) {
      ...
      dropdownEl.removeAttribute('data-dropdown-placement');
      dropdownEl.removeAttribute('data-dropdown-align');
    }
  }
  ...
  function createDropdown(labelText, type = 'default') {
    ...
    if (labelText) {
      ...
      label.className = 'gurum-dropdown-label';
  ...
  function createHoverToolbarElement() {
    ...
    toneSection.className = 'gurum-section gurum-section--tone';
    const toneLabelEl = document.createElement('span');
    toneLabelEl.className = 'gurum-section-label';
    toneLabelEl.textContent = '답변 톤';
    const toneDropdown = createDropdown('', 'tone');
    const toneButtons = new Map();
    const toneList = document.createElement('div');
    toneList.className = 'gurum-tone-options';
    HOVER_TONE_PRESETS.forEach((preset) => {
      ...
      toneButtons.set(preset.id, btn);
      toneList.appendChild(btn);
    });
    toneDropdown.panel.appendChild(toneList);
    toneDropdown.value.textContent = getToneLabel(hoverToolbarState.tone);

    const timestampSection = document.createElement('div');
    timestampSection.className = 'gurum-section gurum-section--timestamp';
    const timestampLabelEl = document.createElement('span');
    timestampLabelEl.className = 'gurum-section-label';
    timestampLabelEl.textContent = '타임스탬프';
    const timestampDropdown = createDropdown('', 'timestamp');
    const timestampButtons = new Map();
    const timestampList = document.createElement('div');
    timestampList.className = 'gurum-prompt-list';
    const timestampPresets = [
      { id: 'include', label: '시간 포함', value: true },
      { id: 'exclude', label: '포함 안함', value: false },
    ];
    timestampPresets.forEach((preset) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      ...
      timestampButtons.set(preset.id, btn);
      timestampList.appendChild(btn);
    });
    timestampDropdown.panel.appendChild(timestampList);
    timestampDropdown.value.textContent = hoverToolbarState.includeTimestamp
      ? '시각 포함'
      : '포함 안함';

    const promptSection = document.createElement('div');
    promptSection.className = 'gurum-section gurum-section--prompt';
    const promptLabelEl = document.createElement('span');
    promptLabelEl.className = 'gurum-section-label';
    promptLabelEl.textContent = '커스텀 프롬프트';
    const promptDropdown = createDropdown('', 'prompt');
    const promptButtons = new Map();
    const promptList = document.createElement('div');
    promptList.className = 'gurum-prompt-list';
    ...
    promptButtons.set('none', noneBtn);
    promptList.appendChild(noneBtn);
    ...
    HOVER_CUSTOM_PROMPTS.forEach((preset) => {
      ...
      promptButtons.set(preset.id, btn);
      promptList.appendChild(btn);
    });

    promptDropdown.panel.appendChild(promptList);
    promptDropdown.value.textContent = getPromptLabel(hoverToolbarState.selectedPromptId);

    toneSection.append(toneLabelEl, toneDropdown.root);
    timestampSection.append(timestampLabelEl, timestampDropdown.root);
    promptSection.append(promptLabelEl, promptDropdown.root);

    groups.append(timestampSection, toneSection, promptSection);
